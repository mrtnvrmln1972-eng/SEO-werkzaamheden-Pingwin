...
const T = {
  font: "Montserrat",
  color: {
    accent:      "F15829",   // --wpex-accent op pingwin.nl
    accentAlt:   "E7773F",   // warm oranje van de menu-CTA
    accentHover: "C95E28",
    accentSoft:  "FFF4EE",
    red:         "A22F3B",
    blue:        "0E7E9A",
...
function createKPITable(metrics) {
  ...
  // Nederlandse labels in de statuskolom; m.statusLabel kan overrulen.
  const statusLabelNL = {
    good:    "GOED",
    warn:    "LET OP",
    bad:     "ACTIE",
        ...
        new TableCell({
          ...
          children: [
            new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [
                baseRun(m.statusLabel || statusLabelNL[m.status] || "NEUTRAAL", {
                  bold: true,
                  color: T.color.white,
                  tracking: 8,
                }),
...
/* =========================================================
 * 12b. COMPONENT: createImageBlock
 *
 * Afbeelding (screenshot, grafiek) over de volle contentbreedte, met
 * optioneel bijschrift in klein grijs cursief eronder. Schaal wordt
 * berekend uit de originele pixelmaten (verplicht meegeven), zodat de
 * verhouding altijd klopt. type: "png" | "jpg" | "gif" | "bmp".
 * ======================================================= */
function createImageBlock(imageBuffer, { widthPx, heightPx, caption, type = "png" } = {}) {
  if (!widthPx || !heightPx) {
    throw new Error("[pingwin-docx] createImageBlock: geef widthPx en heightPx van de originele afbeelding mee.");
  }
  // Contentbreedte in px bij 96 dpi: 9746 DXA / 1440 * 96 ≈ 650 px.
  const MAX_W = 650;
  const w = Math.min(widthPx, MAX_W);
  const h = Math.round((heightPx / widthPx) * w);

  const blocks = [
    new Paragraph({
      spacing: { before: 120, after: caption ? 60 : 160 },
      alignment: AlignmentType.CENTER,
      children: [
        new ImageRun({
          data: imageBuffer,
  ...
  if (caption) {
    blocks.push(
      new Paragraph({
        ...
        alignment: AlignmentType.CENTER,
        children: [
          baseRun(caption, { italic: true, color: T.color.brown, size: T.size.small }),
        ],
      })
    );
  }
...
/* =========================================================
 * 13. VANGNET: flattenChildren
 *
 * Sommige componenten (createCoverPage, createSectionDivider,
 * createHighlightBox, createBulletList) retourneren een ARRAY van
 * elementen. Belandt zo'n array zonder spread (...) in children, dan
 * serialiseert docx hem als ongeldige XML (<0/>) en weigert Word het
 * bestand te openen. Deze functie maakt dat onmogelijk: hij flattent
 * geneste arrays, gooit null/undefined/false weg en faalt LUID (met
 * duidelijke melding) op alles wat geen docx-component is, zodat de
 * fout bij het bouwen zichtbaar wordt in plaats van bij de klant.
 * ======================================================= */
function flattenChildren(children) {
  ...
  for (const child of input) {
    ...
    if (Array.isArray(child)) {
      ...
      throw new Error(
        `[pingwin-docx] Ongeldig element in children: ${JSON.stringify(child)} (type ${typeof child}). ` +
        "Alleen docx-componenten (Paragraph, Table, ...) zijn toegestaan. " +
        "Dit zou een corrupt .docx-bestand opleveren dat Word niet kan openen."
      );
    }
  }
  return flat;
...
/* =========================================================
 * 14. COMPONENT: createPingwinDocument
 *
 * DE aanbevolen manier om een Pingwin-document te bouwen. Regelt A4,
 * marges, header, footer en styles, en haalt children altijd door
 * flattenChildren. Hierdoor maakt het niet meer uit of een component
 * met of zonder spread (...) wordt doorgegeven: het resultaat is
 * altijd een geldig .docx-bestand.
 * ======================================================= */
function createPingwinDocument({ klant, rapporttype, logoBuffer, children }) {
  return new Document({
    styles,
    sections: [{
      properties: {
      ...
      headers: { default: createPageHeader(klant, rapporttype, logoBuffer) },
