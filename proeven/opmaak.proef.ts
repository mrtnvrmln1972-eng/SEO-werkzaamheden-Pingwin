...
for (const vol of alleSchermen(path.join(WORTEL, "app"))) {
  ...
  regels.forEach((regel, i) => {
    ...
    // Een <script> zet niets op het scherm en een <style> zet geen tekst maar
    // opmaak (de vastgelegde huisstijl in app/layout.tsx), dus de regel over
    // gerenderde tekst gaat er niet over. Alleen deze twee vormen zijn
    // vrijgesteld; elke andere dangerouslySetInnerHTML is nog steeds tekst in
    // beeld en moet door mdToHtml.
    const isOpmaakTag = /<(script|style)\b/.test(regel);
    if (regel.includes("dangerouslySetInnerHTML") && !isOpmaakTag && !GERENDERD.test(regel) && !GERENDERD.test(regels[i + 1] || "")) {
...
const NAAMRUIMTEN = ["at-", "bulk-", "kw-", "wl-", "wp-", "gmb-", "hm-", "om-", "zp-", "prio-", "tw-", "gp-"];
