...
export async function GET(req: NextRequest) {
  ...
  if (!slug || !url) return NextResponse.json({ ok: false, error: "Klant en URL zijn verplicht." }, { status: 400 });
  const g = await guardSlug(req, slug); if (!g.ok) return g.res;
  const [run, everDone, everLinks] = await Promise.all([getLatestDocRun(slug, url), getStepsEverDone(slug, url), getStepLinks(slug, url)]);
  return NextResponse.json({ ok: true, run, everDone, everLinks });
}
