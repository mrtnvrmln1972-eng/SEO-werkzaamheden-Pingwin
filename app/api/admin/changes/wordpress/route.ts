...
export async function POST(req: NextRequest) {
  ...
  // In een try, zodat een echte fout (timeout, 401, geblokkeerde API) leesbaar
  // teruggegeven wordt i.p.v. een generieke "mislukt".
  try {
    const creds = await getWpCreds(slug);
    if (creds) {
      const res = await addWordpressRevisions(slug, client.domain, creds);
      if (!res.hasApi) return NextResponse.json({ ok: false, error: "Kon de revisies niet ophalen. Controleer of de inloggegevens kloppen (applicatie-wachtwoord, gebruiker met bewerkrechten) en of de site WordPress is met een open REST API." }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, error: `Ophalen uit WordPress mislukt: ${e instanceof Error ? e.message : "onbekende fout"}` }, { status: 502 });
  }
}
