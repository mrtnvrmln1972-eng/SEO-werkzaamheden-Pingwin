...
export async function GET(req: NextRequest) {
  ...
  if (!gekoppeld) {
    return NextResponse.json({
      ok: true, gekoppeld: false, instelling, pijplijnen: [], velden: [], eigenaren: [], leads: [],
...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => ({}))) as {
    ...
    bron?: "contacten" | "deals"; filterVeld?: string; filterWaarde?: string; eigenaar?: string;
  ...
  if (body.actie === "instellingen") {
    ...
    await saveHubspotInstelling({
      ...
      ...(body.eigenaar !== undefined ? { eigenaar: body.eigenaar } : {}),
