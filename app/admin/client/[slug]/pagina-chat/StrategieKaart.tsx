      ...
      <div className="page-chat-log">
            ...
            return (
              <div key={i} className="pch-msg-edit">
                ...
                <div className="pch-msg-edit-actions">
                  <button type="button" className="btn btn-klein" onClick={() => saveEdit(i)}>Opslaan</button>
                  <button type="button" className="btn btn-klein" onClick={() => setEditIdx(null)}>Annuleren</button>
                </div>
              </div>
            );
          }
        ...
        {busy && (
          <div className="page-chat-msg assistant muted" style={{ display: "flex", alignItems: "center", gap: "var(--s-3)" }}>
            ...
            <button type="button" className="btn btn-klein" onClick={cancelSend}
        ...
        <>
          ...
          <div className="page-chat-tools">
            ...
            {taskDone && !planDone && lastAssistant && (
              <button type="button" className="btn btn-klein" onClick={() => void acceptPlan(lastAssistant)}
            ...
            {lastAssistant && (
              <button type="button" className={"btn btn-klein" + (taskGen ? " busy" : "")} onClick={() => ensureDriveMap(() => void makeWorkItem())} disabled={taskGen}>{taskGen ? "Document maken…" : "Document opnieuw maken"}</button>
            )}
  ...
  return (
      <div className={"page-chat step-card step-card-2" + (planDone || taskDone ? " done" : "")}>
        <div className="step-head" onClick={() => setChatOpen((o) => !o)} title={chatOpen ? "Chat inklappen" : "Chat uitklappen"}>
          ...
          {chatOpen && (chats.length > 0 || msgs.length > 0) && <span className="step-head-right"><button type="button" className="btn btn-klein" onClick={(e) => { e.stopPropagation(); newChat(); }}>+ Nieuwe chat</button></span>}
        </div>
