...
- Per geraakte pagina wordt het advies samengevat dat specifiek over díe pagina gaat: de bedoelde rol, het primaire zoekwoord, de actie en eventuele interne-link-afspraken.
...
Cannibalisatie ontstaat meestal niet door slechte analyses maar door losse beslissingen die elkaar tegenspreken. Door de clusterbeslissing één keer te nemen en expliciet door te geven, blijft de eigenaar-keuze per zoekintentie overal consistent; precies wat Google nodig heeft om één duidelijke pagina per intentie te kunnen belonen."} /></span>
      ...
      <div className={"page-chat-docs step-card step-card-4" + (allStepsDone ? " done" : "")}>
          <div className="step-head" onClick={() => setVervolgOpen((o) => !o)}>
            ...
            <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 3 — Documenten (analyse, blauwdruk, copy)" text={"De vastgelegde strategie wordt hier uitgewerkt tot drie documenten die op elkaar voortbouwen: de blauwdruk leest de analyse, de copy leest blauwdruk én analyse. Elke stap draait op de achtergrond, landt als net Pingwin-document in de Drive-klantmap (of als download) en wordt een afgeronde werkzaamheid.
## 1. SEO-analyse: een audit tegen meetbare criteria
- **Databronnen:** het plan, de echt gemeten pagina (koppen, meta, woordenaantal, keyword-density, FAQ, schema, interne links), de Search Console-rankings, Core Web Vitals uit PageSpeed, én Ahrefs: waarop de URL nu rankt, de top-10 van het primaire zoekwoord (met Domain Rating per concurrent) en semantische varianten.
- **Vaste secties,** van KPI's en quick wins (posities 4-20) tot structuur, content, zoekintentie-toets tegen de SERP, FAQ/AEO en E-E-A-T; afgesloten met een scorecard en prioriteiten.
- **Hard oordeel:** de pagina krijgt een gate-verdict; PASS vereist nul kritieke punten, hooguit twee grote, en een score van minimaal 85.
- **Behoud is het uitgangspunt:** de analyse benoemt expliciet wat er goed is en moet blijven (koppen, alinea's, tabellen); er wordt alleen verandering geadviseerd waar de criteria of de top-10-analyse dat vereisen. Dat beschermt bestaande rankings.
## 2. Blauwdruk: de bouwtekening, gebaseerd op de top-10
- De structuur wordt **gebaseerd op de top-10-analyse** van het gekozen zoekwoord; de drie best scorende concurrenten worden daarbij echt uitgemeten: hun woordenaantal, H1, complete koppenlijst, FAQ en schema. Zo zie je welke invalshoeken winnaars behandelen én wat onze pagina mist.
- **Concreet en meetbaar:** H2-dekking van het zoekwoord tussen 60 en 80% (doel 70%; hoger oogt onnatuurlijk en telt als stuffing), plaatsnaam in hooguit 2 à 3 koppen, meta-title 50-60 tekens met het zoekwoord vooraan, meta-description 140-160 tekens, FAQ met 4 tot 6 vragen die de zoekintentie dekken, en een variantenlijst van 10-15 semantische termen.
- **Onderscheid als eis:** naast alles wat de top-10 dekt schrijft de blauwdruk voor wat de pagina uniek maakt; de reden waaróm Google hem in die top-10 zou zetten, in plaats van een kopie van nummer drie.
## 3. Copy: de volledig uitgeschreven pagina
- Geen samenvatting maar de **daadwerkelijke paginatekst**: reken op 6 tot 9 H2-secties met volle alinea's van 80 tot 150 woorden, een uitgebreide FAQ van 6 tot 8 vragen met echte antwoorden van 40 tot 80 woorden (gericht op long-tail), het primaire zoekwoord in de eerste 100 woorden, een natuurlijke dichtheid van 0,5 tot 2%, en minstens 60% van de varianten verwerkt.
- Geschreven in de **tone of voice** uit het klantprofiel, met een automatische nacontrole die te veel herhaling van een naam of plaats in de koppen detecteert en herschrijft.
## Klantversie en interne versie
Standaard krijg je de korte, begrijpelijke klantversie. Onder elke stap kun je de **interne, uitgebreide versie** maken: het volledige technische document met scorecard, criteria-codes en tekencounts, voor eigen onderbouwing of overdracht aan een specialist."} /></span>
          </div>
          {vervolgOpen && (
      ...
      <div className={"page-chat-canni step-card step-card-5" + (canniDone ? " done" : "")}>
        <div className="step-head" onClick={() => setCanniOpen((o) => !o)}>
          ...
          <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 4 — Kannibalisatie oplossen" text={"Kannibalisatie betekent dat meerdere eigen pagina's om dezelfde zoekintentie vechten; Google kan dan niet kiezen en geen van de pagina's haalt zijn potentie. Deze stap brengt dat voor deze pagina volledig in kaart en levert per betrokken pagina één onderbouwde actie.
## Het principe: GSC is de waarheid, Ahrefs de verrekijker
- **Search Console bewijst:** splitsen meerdere eigen pagina's de klikken en vertoningen op letterlijk dezelfde zoekopdracht, dan is dat harde kannibalisatie; dit signaal weegt het zwaarst. Per zoekwoord staat er expliciet bij welke andere eigen pagina's Google óók toont.
- **Ahrefs ontdekt:** waarop rankt deze pagina, welke eigen pagina's ranken op de kernterm, en (per beslis-zoekwoord) het echte volume plus de complete top-10; ook 'verstopte kapers' worden gevonden, pagina's die je niet verdenkt maar wél op de term ranken.
- **Nuance ingebouwd:** een blog naast een transactionele pagina met een andere intentie is géén kannibalisatie en wordt niet geflagd; taalvarianten krijgen nooit een redirect-advies maar hreflang.
## De scheidsrechter per zoekwoord
Voor elke betwiste term wordt beslist: verdient hij een eigen pagina of hoort hij bij deze pagina geclusterd? Volume rond nul en geen eigen pagina's in de top-10 betekent clusteren; echt volume mét eigen pagina's in de top-10 rechtvaardigt een eigen pagina; en tonen twee termen voor meer dan 50% dezelfde top-10-URL's, dan is het één intentie en dus één pagina.
...
## Wat je terugkrijgt
Een gerangschikte lijst kansen met per bron de onderbouwing (relevantie, verwijzende domeinen, verkeer), de voorgestelde ankertekst en, als WordPress gekoppeld is, een directe bewerk-link naar die pagina in de backend; plaatsen is dan een minuut werk. Overnemen maakt er een net werkdocument en één developer-taak van."} /></span>
        </div>
...
## De aanpak: één samenhangende entity graph
- Geen losse snippets, maar één **@graph** waarin elke pagina met vaste @id's verwijst naar het site-brede identiteitsblok (organisatie + website op de homepage). Elke pagina krijgt sowieso een WebPage- en BreadcrumbList-node; de hoofdentiteit hangt daar via @id aan vast. Zo bouw je één consistente entiteit op in plaats van honderd losse claims.
- Het **bedrijfstype** uit de bevestigde Bedrijfsgegevens (Klant-tab) bepaalt de keuze: kliniek wordt MedicalClinic, lokaal bedrijf LocalBusiness, dienstverlener Service met provider en werkgebied, blog een Article met auteur en datums; en nooit Product-markup op een niet-productpagina.
## De kwaliteitsregels
...
- **Bestaand plugin-schema wordt gedetecteerd** (Yoast, RankMath en dergelijke) en het advies vult aan zonder te dupliceren: één instantie per type per pagina.
- **Eerlijk over de opbrengst:** FAQ-rich-results zijn door Google gestopt, dus die worden nergens beloofd; wél worden pagina's met een rijke, kloppende entity graph aantoonbaar vaker geciteerd door AI-assistenten, en de zichtbare content blijft altijd het belangrijkst.
## Wat 'overnemen' oplevert
Drie dingen tegelijk: een kort uitleg-document voor klant en developer, een **los .json-bestand** dat letterlijk te plakken is (met verwijzing naar Google's Rich Results Test en validator.schema.org voor de controle), en één developer-taak met alle links en eventuele waarschuwingen erbij."} /></span>
        </div>
