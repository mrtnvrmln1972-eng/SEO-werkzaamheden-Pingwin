    ...
    <div className="page-chat-wrap">
      ...
      <div className={"page-chat step-card step-card-2" + (taskDone ? " done" : "")}>
        <div className="step-head" onClick={() => setChatOpen((o) => !o)} title={chatOpen ? "Chat inklappen" : "Chat uitklappen"}>
          ...
          <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 1 — Strategie voor deze pagina" text={"Alles begint met de strategiebepaling: wat moet deze pagina zijn, op welk zoekwoord, en wat gaan we doen? Deze stap bestaat uit twee delen die bij elkaar horen.\nBovenin: de VASTGELEGDE STRATEGIE (de conclusie), in vier vaste onderdelen: de rol van de pagina, het primaire en secundaire zoekwoord, de acties en de doel-URL. Deze conclusie is leidend voor alles wat volgt: de documenten (analyse, blauwdruk, copy) schrijven ernaartoe, en bij kannibalisatie bepaalt hij wie de eigenaar van een zoekwoord is; niet de toevallige huidige ranking.\nDaaronder: de STRATEGIE-CHAT waarin je die conclusie uitwerkt, als een sparringsessie met een SEO-strateeg die alle data al voor zich heeft:\n- Automatisch bekend: de live status en titel, de Search Console-rankings, de volledige paginalijst, wie op dezelfde zoekwoorden rankt, het klantprofiel en de bestaande taken.\n- Zelf op te zoeken tijdens het gesprek: echte zoekvolumes en intenties, de top-10 per zoekwoord met de autoriteit van concurrenten, backlinks en Domain Rating (van jullie én concurrenten) en de echte inhoud van elke pagina.\nDe werkwijze: stel je vraag ('Hoe staat deze pagina ervoor?', 'Wat is de beste strategie?'), praat door tot je het eens bent, en klik 'Vat samen tot conclusie & strategie' gevolgd door 'Neem plan over'. De conclusie bovenin vult zich dan vanzelf; zelf typen of bijschaven kan altijd via Bewerken."} /></span>
      ...
      <div className={"page-chat-cluster-card step-card step-card-3" + (clusterDone > 0 ? " done" : "")}>
        <div className="step-head" onClick={() => setDoorgevenOpen((o) => !o)}>
          ...
          <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 2 — Doorgeven aan gelieerde pagina's" text={"Een strategie voor één pagina raakt bijna altijd ook andere pagina's: een pagina die hetzelfde zoekwoord kaapt, een locatiepagina die een eigen term moet krijgen, een pagina waarvandaan een interne link moet komen.\nDeze stap regelt dat je dat advies niet kwijtraakt:\n- Het dashboard leest de conclusie van de chat en haalt eruit welke ANDERE pagina's erin genoemd worden, met per pagina het advies dat specifiek over hén gaat.\n- Vink aan welke pagina's het advies moeten krijgen en klik doorgeven.\n- Die pagina's krijgen het advies als vertrekpunt mee ('half plan'): open je daar straks de chat of het plan, dan ligt deze strategie er al als context, inclusief de volledige conclusie van dit gesprek.\nZo werk je cluster voor cluster consistent: de beslissing die je hier neemt, wordt op de andere pagina's niet opnieuw uitgevonden of tegengesproken."} /></span>
        </div>
        {doorgevenOpen && (
      ...
      <div className={"page-chat-docs step-card step-card-4" + (allStepsDone ? " done" : "")}>
          <div className="step-head" onClick={() => setVervolgOpen((o) => !o)}>
            ...
            <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 3 — Documenten (analyse, blauwdruk, copy)" text={"Hier laat je de vastgelegde strategie uitwerken tot drie concrete documenten, in vaste volgorde:\n- SEO-analyse: hoe de pagina er nu voor staat en wat er moet gebeuren, onderbouwd met echte data (rankings, volumes, top-10).\n- Blauwdruk: de bouwtekening van de nieuwe pagina; welke koppen, secties en zoekwoorden waar komen, afgestemd op de zoekintentie en de concurrentie.\n- Copy: de volledig uitgeschreven paginatekst volgens de blauwdruk, in de tone-of-voice van de klant.\nPraktisch:\n- Elke stap draait op de achtergrond (je kunt gewoon doorwerken of het scherm sluiten) en kan enkele minuten duren.\n- Elk document komt netjes opgemaakt in de Drive-klantmap (of als download zonder Drive-koppeling) én als afgeronde werkzaamheid in de takenlijst en het klantdashboard.\n- Standaard krijg je de klantversie; onder elke stap kun je ook een uitgebreide interne versie laten maken (met alle onderbouwing, voor eigen gebruik).\nDe kwaliteit leunt op wat ervoor gebeurde: een goede, doorgesproken strategie (stap 1) en een gevuld klantprofiel maken deze documenten aantoonbaar beter."} /></span>
          </div>
          {vervolgOpen && (
      ...
      <div className={"page-chat-canni step-card step-card-5" + (canniDone ? " done" : "")}>
        <div className="step-head" onClick={() => setCanniOpen((o) => !o)}>
          ...
          <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 4 — Kannibalisatie oplossen" text={"Kannibalisatie betekent dat meerdere eigen pagina's op hetzelfde zoekwoord concurreren; Google kan dan niet kiezen en geen van beide rankt goed. Deze stap brengt dat voor déze pagina in kaart en lost het op.\nWat de analyse doet (op de achtergrond, met echte Ahrefs- en Search Console-data):\n- Per zoekwoord rond deze pagina: welke eigen pagina's ranken er allemaal op, met positie en vertoningen.\n- Per zoekwoord de scheidsrechter-vraag: verdient dit een éigen pagina (genoeg volume, eigen intentie) of hoort het bij deze pagina geclusterd?\n- Welke pagina's deze pagina 'kapen': ze vangen verkeer weg dat volgens het plan hierheen moet.\nWat je terugkrijgt: per betrokken pagina één concrete actie (301-redirect, interne link met ankertekst, herrichten op een eigen term of samenvoegen), klaar om over te nemen als taken.\nDe hoofdregel: de EIGENAAR van een zoekintentie is de pagina waarvan het plan die intentie claimt; de huidige rankings vertellen alleen waar de waarde nu zit die naar de eigenaar toe moet."} /></span>
        </div>
      ...
      <div className={"page-chat-links-card step-card step-card-6" + (ilDone ? " done" : "")}>
        <div className="step-head" onClick={() => setLinksOpen((o) => !o)}>
          ...
          <span onClick={(e) => e.stopPropagation()}><HelpHint xl title="Stap 5 — Interne links naar deze pagina" text={"Interne links zijn de wegwijzers binnen de site: ze sturen bezoekers én autoriteit (linkwaarde) naar de pagina's die het belangrijkst zijn. Deze stap zoekt de beste kansen om NAAR deze pagina te linken.\nHoe de analyse rangschikt (op de achtergrond):\n- Relevantie eerst: de bronpagina moet het onderwerp echt raken, anders telt de link niet.\n- Daarna autoriteit: bronpagina's die zelf externe links (backlinks) hebben, geven het meeste door.\n- En verkeer: bronpagina's met veel Search Console-bezoek leveren ook echte klikken op.\nMenu- en footerlinks tellen niet mee als bestaande link (die staan overal); het gaat om links in de lopende tekst.\nWat je terugkrijgt per kans: de bronpagina, een voorgestelde ankertekst (de klikbare woorden, afgestemd op het zoekwoord van deze pagina) en een directe bewerk-link naar die pagina in de WordPress-backend, zodat het plaatsen een minuut werk is. Overnemen zet de kansen als taken in de lijst."} /></span>
        </div>
