...
import { ensureHeadingLabels, selfCheckCopyHeadings } from "./copy-koppen";
...
async function buildContext(slug: string, url: string, extra?: string): Promise<{ text: string; primary: string; paginaLeeft: boolean }> {
  ...
  // Aangeleverde documenten bij deze pagina: opgezocht, uitgelezen en uitgewerkt
  // tot een briefing, en dan compleet mee de fase in. Zonder deze stap ging
  // alleen het ADRES mee en had de blauwdruk de afspraken nooit gezien, terwijl
  // het document er wel gewoon af kwam. Zie lib/plan-bronnen.ts.
  // Ruim begrensd (de route mag 800 seconden), maar niet ongelimiteerd: één
  // hangend document mag de fase niet ophouden.
  const bronDocs = await withTimeout(bronContext(slug, url, extra).catch(() => ""), 180000, "");
