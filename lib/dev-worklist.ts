...
// ═══════════════════════════════════════════════════════════
// WERKLIJST SITEBOUWER (alt-teksten, site-breed)
// ═══════════════════════════════════════════════════════════
// Alt-teksten ontbreken op tientallen pagina's tegelijk, en dat wordt nooit een
// stapel losse kaartjes. Deze motor crawlt de live pagina's, kijkt naar elke
// afbeelding zonder alt-tekst en schrijft er één plakbare regel bij, per
// AFBEELDING (want WordPress bewaart er één per afbeelding, niet per pagina).
// Het resultaat is één deelbare afvinklijst plus één verzamelkaart in de
// weekplanning.
//
// ── Wat hier op 18-08-2026 uit is gegaan, en waarom ──
// 1. De meta's. Die staan in Meta & CTR: dáár ontstaat de tekst, dáár keurt
//    Maarten hem goed, en dáár zit de knop "Doorvoeren op de site". De
//    werklijst haalde ze op en zette ze er nog een keer bij, dus dezelfde
//    goedkeuring stond op twee schermen en je wist nooit welke telde.
// 2. Het Word-document in Drive. Een lijst van tientallen regels in een
//    document wordt niet gelezen en al helemaal niet afgevinkt. De deelbare
//    afvinklijst kan alles wat het document kon, plus bijhouden wat af is en
//    of het echt op de site staat.
// ═══════════════════════════════════════════════════════════
...
// Een pagina in de lijst. De meta-velden die hier stonden (curTitle, newTitle,
// titleStand en hun description-tweelingen) zijn er op 18-08-2026 uit: meta's
// horen in Meta & CTR, daar staat ook de knop om ze door te voeren. Wat een
// pagina hier nog doet is de foto's dragen: hoeveel er zijn en of ze ergens
// over gaan.
...
// Stabiele sleutels: a|<bestand> voor een alt-tekst, i|<bestand> voor de
// indeling van een afbeelding. Er was ook m|<pagina>|title/desc voor de meta's;
// die zijn hier weg, want meta's horen in Meta & CTR.
//
...
// De werklijst gaat helemaal niet meer over meta's.
//
// Eerst schreef hij ze zelf, met een eigen AI-aanroep, en kon dezelfde pagina
// twee verschillende voorstellen hebben. Daarna haalde hij op wat in Meta & CTR
// was goedgekeurd, en zette dat er nog een keer bij. Ook dat is er op
// 18-08-2026 uit: dan staat dezelfde goedkeuring op twee schermen, met twee
// knoppen om hem door te voeren, en weet je nooit welke telt. Meta & CTR is de
// enige plek waar een meta ontstaat, beoordeeld wordt en de site op gaat.
...
export async function runDevWorklist(slug: string): Promise<{ ok: boolean; error?: string }> {
  await ensureSchema();
  await ensureTable();
  ...
  try {
    ...
    for (let i = 0; i < urls.length; i += 6) {
      const batch = await Promise.all(urls.slice(i, i + 6).map(async (u) => {
        ...
        return { url: u.url, path: pagePath(u.url), m, missingAlts: [] } as PageWork;
      }));
      measured.push(...(batch.filter(Boolean) as PageWork[]));
    }

    // 2. Afbeeldingen zonder alt-tekst verzamelen. De gemeten meta gaat en passant
    //    naar Meta & CTR: die motor heeft precies deze meting nodig om te weten wat
    //    er nu op de pagina staat, en hier is hij gratis. Dat is ook de enige
    //    bemoeienis die deze motor nog met meta's heeft.
    ...
    // 4. Een pagina hoort op de lijst als er alt-teksten ontbreken. Verder niets:
    //    een meta die niet deugt is werk voor Meta & CTR, en die heeft zijn eigen
    //    scherm met zijn eigen doorvoerknop.
    const probleem = measured.filter((p) => p.missingAlts.length);
    if (!probleem.length) {
      await setState(slug, "done", "", "Er staat niets klaar voor de bouwer: elke afbeelding op de gemeten pagina's heeft al een alt-tekst.", "");
      return { ok: true };
    }
    ...
    // 6. Hier werd een Word-document in de Pingwin-huisstijl gebouwd en naar
    //    Drive geüpload. Weg op 18-08-2026: een lijst van tientallen regels in
    //    een document leest niemand, en afvinken kan er niet in. De deelbare
    //    afvinklijst hieronder doet hetzelfde en houdt wél bij wat af is en of
    //    het echt op de site staat.
    const totAlt = probleem.reduce((n, p) => n + p.missingAlts.length, 0);
    ...
    const KAART = "Werklijst sitebouwer: alt-teksten site-breed";
    const toel = [
      "Achtergrond:",
      `- ${totAlt} afbeeldingen zonder alt-tekst, verdeeld over ${probleem.length} pagina's.`,
    ...
    if (bestaand[0]) {
      await sql`UPDATE client_weekplan SET toelichting = ${toel}, copy_url = ${null}, updated_at = now() WHERE client_slug = ${slug} AND id = ${bestaand[0].id}`;
    } else {
      ...
      await sql`
        ...
        VALUES (${slug}, 'overzicht', ${KAART}, ${toel}, 'Dev', ${null}, 'overig', ${null}, ${null}, ${w.year}, ${w.week}, 'gepland', 0, now())`;
    }

    const samenvatting = `${totAlt} afbeeldingen zonder alt-tekst, verdeeld over ${probleem.length} pagina's${dubbel.length ? `, en ${dubbel.length} foto's die op meerdere pagina's terugkomen` : ""}.`;
    await setState(slug, "done", "", samenvatting, "");
    return { ok: true };
  } catch (e) {
...
export async function getWorklistData(slug: string): Promise<WorklistData> {
  ...
  //
  // In lijsten die al opgeslagen stonden zitten nog meta-teksten. Die worden
  // hier weggelaten in plaats van meegelezen: meta's horen in Meta & CTR, en een
  // oude lijst hoort ze niet alsnog te tonen (regel: met terugwerkende kracht).
  const pages = ruw.map((p) => ({
    url: p.url,
    path: p.path,
    beeldTotaal: p.beeldTotaal,
    beeldRaak: p.beeldRaak,
    beeldOnderwerp: p.beeldOnderwerp,
...
// "gecontroleerd"-vinkje op elke alt-tekst die echt op de afbeelding staat.
// Werkt ook de dubbel-status bij, zodat de knop
