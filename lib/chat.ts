...
function overviewTools(client: ClientConfig, base: { tools: ToolDef[]; run: ToolRunner }, collected: ProposedAction[]): { tools: ToolDef[]; run: ToolRunner } {
  ...
  const extra: ToolDef[] = [
    ...
    { name: "stel_acties_voor", description: "Stel één of meer concrete acties voor als knopkaartjes die Maarten met één klik kan goedkeuren. VOER NIETS ZELF UIT. Gebruik dit ALLEEN wanneer Maarten er EXPLICIET om vraagt (bijv. 'maak er een kaart/taak van', 'pak dit op', 'zet dit door', 'werk dit uit'). NIET uit jezelf en nooit een hele reeks tegelijk; standaard spar je gewoon in tekst. Maak alleen kaarten voor precies wat Maarten aangeeft.", input_schema: { type: "object", properties: { acties: { type: "array", items: { type: "object", properties: {
      ...
      taken: { type: "array", description: "Alleen bij weekplan_taken: de lijst concrete taken die uit dit gesprek volgen. Geef PER taak ook alle relevante info mee in 'info', zodat Maarten die later kan uitklappen en de taak week in, week uit kan uitwerken.", items: { type: "object", properties: { taak: { type: "string", description: "Korte, concrete taaktitel (één regel). Zet GEEN 'WEEK X' in de titel; de week geef je apart mee in 'week'." }, week: { type: "integer", description: "In welke week deze taak valt: 1 = deze week, 2 = volgende week, enzovoort. Verdeel de taken realistisch over de komende weken." }, info: { type: "string", description: "Alle relevante info bij deze taak: waarom, welke zoektermen/pagina, de aanpak, de cannibalisatie-nuance, verwachte impact, enzovoort. Nette leesbare tekst met korte kopregels en '-'-bullets (geen #, | of **)." }, wie: { type: "string", enum: ["SEO", "Dev"], description: "Wie voert de taak uit." }, url: { type: "string", description: "Optioneel: de pagina waar de taak over gaat." } }, required: ["taak"] } },
...
export async function answerChat(slug: string, messages: ChatMessage[], thread = "algemeen"): Promise<{ ok: boolean; answer?: string; error?: string; actions?: ProposedAction[]; title?: string; summary?: string }> {
  ...
  const system = isOverview
    ? `Je bent de overkoepelende SEO-strateeg ("bird's eye") van Pingwin voor de klant ${client.name}. Je helpt Maarten vanuit één helder, gestructureerd werkplan bepalen wat we doen, wat er nog moet en waar het laaghangend fruit zit. Dat plan is gegrond in de AFSPRAKEN met de klant (navigatie, zoekwoordenlijst, geplande landingspagina's), niet alleen in snelle winst.\n\n` +
      ...
      `- STAND VAN ZAKEN DEKT ALTIJD DRIE DELEN: (1) wat staat live en hoe presteert het, (2) wat is aangeleverd maar nog niet verwerkt/live, (3) welke pagina's moeten er nog BIJ of uitgebreid worden om de site autoriteit te geven. Deel (3) mag je NOOIT overslaan: haal het uit het blok TE BOUWEN / UIT TE BREIDEN PAGINA'S én uit de AFGESPROKEN ZOEKWOORDEN & LINKS (daar staat vaak een uitgewerkte navigatie met veel geplande pagina's), de STAND VAN ZAKEN en de RECENTE E-MAILS. Noem de te bouwen pagina's concreet (welke pagina/onderwerp, waarom belangrijk voor autoriteit), niet in algemene termen.\n` +
      ...
      `- WEEKPLANNING: vraagt Maarten om een planning of taken (bijv. "kun je een planning maken", "maak een weekplanning", "maak hier taken van", "wat kunnen we oppakken", "geef een werklijstje"), geef dan (a) je inhoudelijke terugkoppeling in tekst (netjes opgemaakt volgens de OPMAAK-regels: blokken met oranje kopjes, streepjes ertussen, bullets, vet, linkjes), met per week kort de context/reden als korte bullets, EN (b) roep in DEZELFDE beurt 'stel_acties_voor' aan met één 'weekplan_taken' waarin de concrete taken staan: per taak de wie (SEO of Dev), de pagina (url), de week (1 = deze week, 2 = volgende, enzovoort) en in 'info' de achtergrond/waarom (waar komt het vandaan, hoe verhoudt het zich tot andere pagina's, wat speelt er). Schrijf de taken NIET óók als tabel/lijst in je tekst; ze verschijnen als kaartjes per week onder je terugkoppeling. CRUCIAAL: vraag NIET eerst "zal ik dit als takenpakket in de weekplanning zetten?" of "wil je dat ik hier kaarten van maak?". De planning-vraag IS het verzoek: maak de kaarten meteen in dezelfde beurt. Kondig het dus niet aan, maak ze echt.\n` +
