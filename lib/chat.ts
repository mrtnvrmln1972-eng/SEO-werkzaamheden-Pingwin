...
function overviewTools(client: ClientConfig, base: { tools: ToolDef[]; run: ToolRunner }, collected: ProposedAction[]): { tools: ToolDef[]; run: ToolRunner } {
  ...
  const extra: ToolDef[] = [
    ...
    { name: "stel_acties_voor", description: "Stel één of meer concrete acties voor als knopkaartjes die Maarten met één klik kan goedkeuren. VOER NIETS ZELF UIT. Gebruik dit ALLEEN wanneer Maarten er EXPLICIET om vraagt (bijv. 'maak er een kaart/taak van', 'pak dit op', 'zet dit door', 'werk dit uit'). NIET uit jezelf en nooit een hele reeks tegelijk; standaard spar je gewoon in tekst. Maak alleen kaarten voor precies wat Maarten aangeeft.", input_schema: { type: "object", properties: { acties: { type: "array", items: { type: "object", properties: {
      ...
      taken: { type: "array", description: "Alleen bij weekplan_taken: de lijst kaarten die uit dit gesprek volgen. VERPLICHT gevuld: roep weekplan_taken NOOIT met een lege of ontbrekende 'taken' aan (dan wordt de actie afgekeurd). Zet in ÉÉN aanroep meteen alles erin, elk met minimaal 'taak', 'week' en 'info'. BUNDEL-REGELS: maak per pagina per week PRECIES ÉÉN kaart; alle deeltaken voor die pagina (meta, alt-teksten, copy, interne links, structured data) zet je als '-'-bullets in 'info' van die ene paginakaart, NIET als losse items. Taken zonder pagina (bijv. een mail sturen) blijven wél een eigen item, zonder url. Streef naar 4 tot 8 kaarten in totaal, maximaal 10; liever één stevige paginakaart met vijf bullets dan vijf losse snippers. Zet in 'info' eerst kort de achtergrond en daarna de deeltaken als bullets, zodat Maarten de kaart kan uitklappen en week in, week uit kan uitwerken.", items: { type: "object", properties: { taak: { type: "string", description: "Korte, concrete taaktitel (één regel). Zet GEEN 'WEEK X' in de titel; de week geef je apart mee in 'week'." }, week: { type: "integer", description: "In welke week deze taak valt: 1 = deze week, 2 = volgende week, enzovoort. Verdeel de taken realistisch over de komende weken." }, info: { type: "string", description: "Alle relevante info/achtergrond bij deze taak: waar komt het vandaan (bijv. de mail van 30 juli), waarom, welke zoektermen/pagina, de aanpak, hoe het zich verhoudt tot andere pagina's, de cannibalisatie-nuance, verwachte impact. Dit is de achtergrond die op de kaart komt, dus volledig. Nette leesbare tekst met korte kopregels en '-'-bullets (geen #, | of **)." }, wie: { type: "string", enum: ["SEO", "Dev"], description: "Wie voert de taak uit." }, url: { type: "string", description: "Optioneel: de pagina waar de taak over gaat." }, taaktype: { type: "string", enum: ["meta", "alt", "copy", "intern", "strategie", "pijplijn", "structured", "overig"], description: "Het type taak, zodat de kaart naar de juiste plek in het dashboard kan deep-linken (meta → Meta & CTR-tab, enz.). meta=meta-title/description, alt=alt-teksten, copy=copy schrijven/controleren, intern=interne links, strategie=strategie bepalen, pijplijn=blauwdruk/copy genereren, structured=structured data, overig=anders." }, bronMail: { type: "string", description: "Optioneel: de webLink van de mail waar deze taak uit voortkomt (die staat bij de RECENTE E-MAILS in de context als 'link: ...'). Zo linkt de kaart direct naar die mail." } }, required: ["taak"] } },
...
async function generateWeekplanActie(client: ClientConfig, context: string, analyse: string, slug: string): Promise<ProposedAction | null> {
  const system =
    ...
    `- Maak per pagina per week PRECIES ÉÉN kaart. Alle deeltaken voor die pagina (meta, alt-teksten, copy, interne links, structured data) zet je NIET als losse kaarten, maar als '-'-bullets in "info" van die ene paginakaart.\n` +
    `- De titel van een paginakaart: het pad plus het doel, bijvoorbeeld "Optimaliseer /hovenier/etten-leur/ (meta, alts, interne links)".\n` +
    `- Taken die niet aan een pagina hangen (bijvoorbeeld "mail Emre over de redirects" of "wacht op de developer") blijven wél een eigen kaart, zonder url.\n` +
    `- Streef naar 4 tot 8 kaarten in totaal, nooit meer dan 10. Liever één stevige paginakaart met vijf bullets dan vijf losse snippers.\n` +
    `- "taaktype" van een gebundelde paginakaart: kies het zwaartepunt (meestal "pijplijn" als er analyse/blauwdruk/copy in zit, anders het meest voorkomende type).\n` +
    `Verder: "week" is een getal (1 = deze week, 2 = volgende, enzovoort); neem de week over zoals de tekst die aangeeft, anders verdeel je realistisch. "wie" is "SEO" of "Dev". "taaktype" is één van: meta, alt, copy, intern, strategie, pijplijn, structured, overig. Zet in "info" eerst kort de achtergrond (waar komt het vandaan, welke zoektermen, verwachte impact) en daarna de deeltaken als '-'-bullets. Verzin geen cijfers; gebruik alleen wat in de tekst staat. Geen emoji, geen Markdown-symbolen (#, | of **), geen tekst buiten de JSON.`;
...
export async function answerChat(slug: string, messages: ChatMessage[], thread = "algemeen"): Promise<{ ok: boolean; answer?: string; error?: string; actions?: ProposedAction[]; title?: string; summary?: string }> {
  ...
  const system = isOverview
    ? `Je bent de overkoepelende SEO-strateeg ("bird's eye") van Pingwin voor de klant ${client.name}. Je helpt Maarten vanuit één helder, gestructureerd werkplan bepalen wat we doen, wat er nog moet en waar het laaghangend fruit zit. Dat plan is gegrond in de AFSPRAKEN met de klant (navigatie, zoekwoordenlijst, geplande landingspagina's), niet alleen in snelle winst.\n\n` +
      ...
      `  - **Vet** voor de kernfeiten (paginanaam, positie, aantallen, prijs, datum). Pagina's/URL's/slugs schrijf je als klikbaar pad (bijv. /lensimplantatie/) of markdown-link.\n` +
