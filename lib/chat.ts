...
function overviewTools(client: ClientConfig, base: { tools: ToolDef[]; run: ToolRunner }, collected: ProposedAction[]): { tools: ToolDef[]; run: ToolRunner } {
  ...
  const extra: ToolDef[] = [
    ...
    { name: "stel_acties_voor", description: "Stel één of meer concrete acties voor die Maarten met één klik kan goedkeuren. VOER NIETS ZELF UIT; dit toont alleen knopkaartjes. Gebruik dit aan het EIND van je antwoord voor de vervolgstappen die je adviseert. Verwijs er in je gewone tekst kort naar (bijv. 'hieronder kun je dit met één klik starten'), maar herhaal de acties niet als lijst.", input_schema: { type: "object", properties: { acties: { type: "array", items: { type: "object", properties: {
      ...
      tekst: { type: "string", description: "Alleen bij profiel_bijwerken: de nuance/tekst die aan het klantprofiel wordt toegevoegd (kort, concreet, in gewone taal). Maarten kan dit op de kaart nog bijstellen." },
...
export async function answerChat(slug: string, messages: ChatMessage[], thread = "algemeen"): Promise<{ ok: boolean; answer?: string; error?: string; actions?: ProposedAction[] }> {
  ...
  const system = isOverview
    ? `Je bent de overkoepelende SEO-strateeg ("bird's eye") van Pingwin voor de klant ${client.name}. Je helpt Maarten vanuit één helder, gestructureerd werkplan bepalen wat we doen, wat er nog moet en waar het laaghangend fruit zit. Dat plan is gegrond in de AFSPRAKEN met de klant (navigatie, zoekwoordenlijst, geplande landingspagina's), niet alleen in snelle winst.\n\n` +
      ...
      `- Zie je in de mail blijvende nuance die het klantprofiel raakt (positionering, terminologie, no-go's, expliciete beslissingen zoals "beplantingsplan geen aparte pagina"), stel dan een 'profiel_bijwerken'-actie voor met een korte, concrete tekst. Zo leert het systeem uit de communicatie en gebruiken copy, meta en strategie de nuance voortaan automatisch. Maarten kan de tekst op de kaart nog bijstellen vóór hij goedkeurt.\n` +
