...
async function generateWeekplanActie(client: ClientConfig, context: string, analyse: string, slug: string): Promise<ProposedAction | null> {
  const system =
    ...
    `Verder: "week" is een getal (1 = deze week, 2 = volgende, enzovoort); neem de startweek over zoals de tekst die aangeeft, anders realistisch. "wie" is "SEO" of "Dev". "taaktype" is één van: meta, alt, copy, intern, strategie, pijplijn, structured, overig. Bouw "info" op in secties, elk met een kort kopje op een eigen regel dat eindigt op een dubbele punt: eerst "Achtergrond:" met 1 tot 3 korte zinnen (waar komt het vandaan, welke zoektermen/cijfers, verwachte impact, eventuele cannibalisatie-nuance), dan "Deeltaken:" met de '-'-bullets, en alleen als er afspraken of bronnen zijn "Afspraken en herkomst:" met '-'-bullets (mail-datum, wie, referenties). Herhaal nooit de kaarttitel als bullet. Verzin geen cijfers; gebruik alleen wat in de tekst staat. Geen emoji, geen Markdown-symbolen (#, | of **), geen tekst buiten de JSON.`;
