...
async function generateWeekplanActie(client: ClientConfig, context: string, analyse: string, slug: string, enkel = false): Promise<ProposedAction | null> {
  const system =
    ...
    `- Site-brede alt-tekst-opruiming over veel pagina's is NOOIT een stapel losse kaartjes: dat wordt ÉÉN Dev-kaart "Werklijst sitebouwer: alt-teksten site-breed" zonder url. Site-brede meta-opruiming is GEEN kaart: die loopt via Meta & CTR. Meta's of alt-teksten van een pagina die tóch een eigen paginakaart heeft blijven gewoon een fase-bullet op die kaart.\n` +
...
export async function oogstTaken(slug: string, thread = "overzicht"): Promise<{ ok: boolean; oogst?: OogstResultaat; error?: string }> {
  ...
  const system =
    ...
    `- Site-brede alt-tekst-opruiming over veel pagina's is ÉÉN Dev-taak "Werklijst sitebouwer: alt-teksten site-breed" zonder url, nooit een stapel losse kaartjes; meta's lopen via Meta & CTR.\n` +
